...
public class SmallChatHistory {
    ...
    public void OnEditMessageButtons(InlineKeyboardButton[][] buttons) =>
        SaveOutput($"Edit buttons [{buttons.Length} lines]");
    ...
    public void OnOutputMessage(string message, InlineKeyboardButton[] buttons) =>
        SaveOutput($"msg: {message} [{buttons.Length}]");

    public void OnOutputMessage(string message, InlineKeyboardButton[][] buttons) =>
        SaveOutput($"msg: {message} [{buttons.Length} lines]");

    public void OnOutputMarkdownMessage(Markdown message, InlineKeyboardButton[] buttons) =>
        SaveOutput($"msg mrkd: {message.GetMarkdownString()} [{buttons.Length}]");

    public void OnOutputMarkdownMessage(Markdown message, InlineKeyboardButton[][] buttons) =>
        SaveOutput($"msg mrkd: {message.GetMarkdownString()} [{buttons.Length} lines]");
